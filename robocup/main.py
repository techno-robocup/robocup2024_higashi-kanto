...
def getline():
    ...
    _COLORSENSOR_L_AVGSUM = (
        (_COLORSENSOR_L_AVGSUM) *
        (__CONST__CLOCK__ - 1) + _COLORSENSOR_L_SUM) // __CONST__CLOCK__
    _COLORSENSOR_R_AVGSUM = (
        (_COLORSENSOR_R_AVGSUM) *
        (__CONST__CLOCK__ - 1) + _COLORSENSOR_R_SUM) // __CONST__CLOCK__
    _I_SUM += _COLORSENSOR_L_SUM - _COLORSENSOR_R_SUM
    _D_SUM = (_COLORSENSOR_L_SUM_BEFORE - _COLORSENSOR_R_SUM_BEFORE) - (
        _COLORSENSOR_L_SUM - _COLORSENSOR_R_SUM)
...
def isgreen_L(r, g, b) -> bool:
    ...
    if (max(r, g, b) == r and max(r, g, b) - min(r, g, b) != 0):
        ...
        if (140 < _COLORSENSOR_L_HSV < 150
                and 16 > max(r, g, b) / 255 * 100 > 10
                and 31 <= _COLORSENSOR_L_G < 50):
            return True
        return False
...
def isgreen_R(r, g, b) -> bool:
    ...
    if (max(r, g, b) == r and max(r, g, b) - min(r, g, b) != 0):
        ...
        if (127 < _COLORSENSOR_R_HSV < 132
                and 17 > max(r, g, b) / 255 * 100 > 13
                and 39 <= _COLORSENSOR_R_G <= 43):
            return True
        return False
    if (max(r, g, b) == g and max(r, g, b) - min(r, g, b) != 0):
    ...
    if (max(r, g, b) == b and max(r, g, b) - min(r, g, b) != 0):
        ...
        if (_COLORSENSOR_R_HSV < 0):
            _COLORSENSOR_R_HSV += 360
        if (128 < _COLORSENSOR_R_HSV < 131
                and 17 > max(r, g, b) / 255 * 100 > 14
                and 39 <= _COLORSENSOR_R_G <= 43):
            return True
        return False
    return False
...
while True:
    ...
    if (not (__DEBUG__COLORSENSOR__GREEN__)):
        __CONST__MOTOR__L__.run(__CONST__SPEED__ * (50 - abs(_D_SUM)) / 40 +
                                __CONST__PROPORTION__ *
                                (_COLORSENSOR_L_SUM - _COLORSENSOR_R_SUM) +
                                _I_SUM * __CONST__I__ + _D_SUM * __CONST__D__)
        __CONST__MOTOR__R__.run(__CONST__SPEED__ * (50 - abs(_D_SUM)) / 40 +
                                __CONST__PROPORTION__ *
                                (_COLORSENSOR_R_SUM - _COLORSENSOR_L_SUM) -
                                _I_SUM * __CONST__I__ - _D_SUM * __CONST__D__)
    if __DEBUG__COLORSENSOR__GREEN__:
        print(
            _COLORSENSOR_L_HSV, _COLORSENSOR_L_G, _COLORSENSOR_R_HSV,
            _COLORSENSOR_R_G,
            max(_COLORSENSOR_L_B, _COLORSENSOR_L_G, _COLORSENSOR_L_R) / 255 *
            100,
            max(_COLORSENSOR_R_B, _COLORSENSOR_R_G, _COLORSENSOR_R_R) / 255 *
            100)
        ...
        if isgreen_R(_COLORSENSOR_R_R, _COLORSENSOR_R_G, _COLORSENSOR_R_B):
            print("RIGHT GREEN")
            ev3.speaker.beep()
    if isgreen_L(_COLORSENSOR_L_R,
                 _COLORSENSOR_L_G, _COLORSENSOR_L_B) and isgreen_R(
                     _COLORSENSOR_L_R, _COLORSENSOR_L_G, _COLORSENSOR_L_B):
    ...
    if(abs(_D_SUM) > 15):
        ...
        if(isgreen_L(_COLORSENSOR_L_R,_COLORSENSOR_L_G,_COLORSENSOR_L_B) or isgreen_R(_COLORSENSOR_L_R,_COLORSENSOR_L_G,_COLORSENSOR_L_B)):
            ...
            time.sleep(0.4)
    """
    if isred_L(_COLORSENSOR_L_R,
               _COLORSENSOR_L_G, _COLORSENSOR_L_B) and isred_R(
                   _COLORSENSOR_R_R, _COLORSENSOR_R_G, _COLORSENSOR_R_B):
